...
import { getExamDocuments } from "../actions/get-exam-documents";
import { uploadExamDocument } from "../actions/upload-document";
import { deleteExamDocument } from "../actions/delete-document";

export const ExamDocumentSection = ({ examId }: { examId: string }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (docId: string) => deleteExamDocument(docId, examId),
    onSuccess(data) {
      if (data.success) {
        toast.success(data.message);
        queryClient.invalidateQueries({ queryKey: ["exam-documents", examId] });
  ...
  return (
    <div className="space-y-3">
      {isMutating && <Skeleton className="h-20 w-full rounded-2xl" />}
          ...
          <div key={doc.id} className="flex items-center gap-2">
            <div className="flex-1">
              <DocumentCard
                ...
                mimeType="PDF"
                onDownload={() =>
                  window.open(`/api/exams/${examId}/documents/${doc.id}/download`, "_blank")
                }
              />
            </div>
            <button
